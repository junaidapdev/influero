-- Atomicity test for mark_payment_received — Feature 11's ACCEPTANCE GATE.
-- Run in the Supabase SQL editor AFTER applying 0006 + 0007. Repeatable: the
-- whole script runs in one transaction and ends with ROLLBACK, so it leaves
-- no data behind. Watch the Messages/NOTICE output — every phase prints PASS
-- or raises TEST FAILED.
--
-- What it proves: forcing a failure BETWEEN the RPC's two steps (payment →
-- received, deal → paid) leaves NEITHER row changed. The fault is a temporary
-- CHECK constraint that rejects ad_deals.status = 'paid', so step 2 raises
-- and Postgres rolls the already-applied step 1 back with it.
--
-- What it does NOT prove: RLS. The SQL editor runs as the table owner, which
-- bypasses RLS — ownership here flows through the RPC's explicit
-- `user_id = auth.uid()` predicates (auth.uid() reads the JWT claims we set
-- below). The 2nd-user RLS test happens live from the app, as with every
-- data-bearing feature.

begin;

-- Simulate the signed-in user: auth.uid() reads request.jwt.claims ->> 'sub'.
-- Uses the oldest auth.users row (the developer's account); transaction-local.
select set_config(
  'request.jwt.claims',
  json_build_object(
...
-- ---------------------------------------------------------------------------
-- Phase 1 — first installment received: payment flips, deal does NOT (one
...
begin
  ...
  raise notice 'PASS [phase 1]: partial payment received, deal untouched';
end $$;

-- ---------------------------------------------------------------------------
-- Phase 2 — FAULT INJECTION (the gate). Reject status='paid' so the RPC's
-- second step fails, then verify the FIRST step rolled back with it: the
-- payment must still be pending. NOT VALID = enforced for new writes only.
-- ---------------------------------------------------------------------------
...
exception
  when check_violation then
    raise notice 'PASS [phase 2a]: injected fault raised inside the RPC';
end $$;
...
begin
  ...
  raise notice 'PASS [phase 2b]: neither the payment nor the deal changed — atomic';
end $$;
...
begin
  ...
  raise notice 'PASS [phase 3]: final installment flipped payment and deal together';
end $$;

-- ---------------------------------------------------------------------------
-- Phase 4 — cancelled is terminal: the payment is recorded, the deal stays
-- cancelled, deal_paid=false (Decision 2).
-- ---------------------------------------------------------------------------
do $$
declare v jsonb;
begin
  ...
  raise notice 'PASS [phase 4]: cancelled deal survived — payment recorded, status terminal';
end $$;
...
exception
  when raise_exception then
    ...
    raise notice 'PASS [phase 5]: already-received payment rejected';
end $$;

do $$ begin raise notice 'ALL PHASES PASSED — rolling back test data'; end $$;
