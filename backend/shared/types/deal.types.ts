...
//   effective_post_at      — GENERATED by Postgres (0028), never written:
//                            coalesce(posted_at, post_date, created_at). The one
//                            answer to "which month does this deal count in",
//                            read by the dashboard/reports RPCs and the /deals
//                            month filter. Never null, so every deal belongs to
//                            exactly one month even with no planned post date.
export type Deal = {
  id: string;
  user_id: string;
  brand_id: string;
